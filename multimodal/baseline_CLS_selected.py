...
def load_and_merge(structured_path, cls_path, selected_variables_path=None):
    """Load structured and CLS data, apply variable filtering, merge, and expand CLS embeddings."""
    ...
    # Filter structured data based on selected variables if provided
    if selected_variables_path:
        if selected_variables_path.endswith(".csv"):
            selected_vars_df = pd.read_csv(selected_variables_path)
        else:
            selected_vars_df = pd.read_excel(selected_variables_path)
        selected_vars = selected_vars_df['Unnamed: 0'].tolist()
        selected_vars = ['NO_SEJOUR', 'y'] + selected_vars
        df_struct = df_struct[[col for col in selected_vars if col in df_struct.columns]]

    # Merge structured and CLS data
    df = pd.merge(df_struct, df_cls[['NO_SEJOUR', 'CLS']], on='NO_SEJOUR')

    # Expand CLS embeddings into individual columns
...
def run_cross_validation(df, output_dir, n_splits=5, random_state=42):
    ...
    for fold, (train_idx, test_idx) in enumerate(tqdm(skf.split(X, y), total=n_splits, desc="Cross-validation")):
        ...
        # Structured features: impute + scale
...
def main():
    ...
    parser.add_argument('--variable_list', default=None, help='Path to variable list CSV or Excel file')
    ...
    df = load_and_merge(args.structured, args.cls, args.variable_list)
