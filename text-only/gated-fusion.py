...
class GatedFusionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return {
            "cls_embeddings": torch.tensor(self.cls_embeddings[idx], dtype=torch.float),
            "entity_embeddings": torch.tensor(self.entity_embeddings[idx], dtype=torch.float),
            "labels": torch.tensor(self.labels[idx], dtype=torch.long)
        }

class GatedFusionDataCollator:
    def __call__(self, features):
        cls_embeds = torch.stack([f["cls_embeddings"] for f in features])
        entity_embeds = torch.stack([f["entity_embeddings"] for f in features])
        labels = torch.tensor([f["labels"] for f in features], dtype=torch.long)
        return {"cls_embeddings": cls_embeds, "entity_embeddings": entity_embeds, "labels": labels}
...
class GatedFusionModel(PreTrainedModel):
    ...
    def __init__(self, config, class_weights=None):
        ...
        # Gating layer to learn importance of CLS vs entity embeddings
    ...
    def forward(self, cls_embeddings=None, entity_embeddings=None, labels=None):
        ...
        fused_embedding = gate * cls_embeddings + (1 - gate) * entity_avg
...
# -------------------- Cross-Validation --------------------
def run_cv_gated(df, entity_cols, metrics_path, folds=N_FOLDS, projection="pca"):
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(cls_embeddings, y)):
        ...
        train_dataset = GatedFusionDataset(X_cls_train, X_ent_train, y_train)
        test_dataset = GatedFusionDataset(X_cls_test, X_ent_test, y_test)

        config = GatedFusionConfig(hidden_size=X_cls_train.shape[1], num_entities=X_ent_train.shape[1], num_labels=2)
        model = GatedFusionModel(config, class_weights=class_weights)

        args = TrainingArguments(
            output_dir=f"./results_gated/fold_{fold}",
        ...
        trainer = Trainer(
            ...
            data_collator=GatedFusionDataCollator(),
...
if __name__ == "__main__":
    ...
    results = run_cv_gated(df, entity_cols, metrics_path=args.metrics_path, folds=args.folds, projection=args.projection)
